...
function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // 直接设置 URL，<img> 标签会自动开始下载图片
      setImage(url);
      // 注意：这里我们不立即关闭 loading，等 <img> 的 onLoad 事件触发后再关闭
    } catch (e: any) {
      alert("生成请求失败，请稍后重试");
      setLoading(false);
    }
  ...
  return (
    <div className="flex min-h-screen bg-[#f0f2f5] text-[#1a1a1a]">
      {/* 侧边栏 */}
      <aside className="w-20 bg-white/70 backdrop-blur-xl border-r border-gray-200 flex flex-col items-center py-10 gap-8 sticky top-0 h-screen z-20">
        ...
        <nav className="flex flex-col gap-6 flex-1 text-gray-400">
          ...
          <button onClick={() => setActiveTab('ppt')} className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${activeTab === 'ppt' ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-gray-100 hover:text-blue-500'}`}>
            <Presentation size={24} />
          </button>
          <button onClick={() => setActiveTab('paper')} className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${activeTab === 'paper' ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-gray-100 hover:text-blue-500'}`}>
            <FileText size={24} />
          </button>
        </nav>
        ...
        <header className="h-24 px-10 flex items-center justify-between sticky top-0 bg-[#f0f2f5]/90 backdrop-blur-md z-10">
          <div className="flex-1 max-w-3xl relative">
            ...
            <button 
              ...
              className="absolute right-2.5 top-2.5 bottom-2.5 w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center text-white"
            >
          ...
          <div className="flex-1 bg-white rounded-[2.5rem] shadow-sm border border-white p-8 flex items-center justify-center relative overflow-hidden">
            {image ? (
              <div className="relative group max-h-full">
                <img 
                  src={image} 
                  className="max-w-full max-h-[65vh] rounded-3xl shadow-2xl transition-opacity duration-500" 
                  alt="AI Generated" 
                  onLoad={() => setLoading(false)} // 图片加载完成后才关闭加载状态
                />
              ...
              <div className="flex flex-col items-center gap-4 text-gray-300">
                ...
                <p className="text-gray-400">
                  {activeTab === 'image' ? '请输入提示词点击生成' : '该功能正在接入中...'}
                </p>
              </div>
            )}
            ...
            {loading && (
              <div className="absolute inset-0 bg-white/60 backdrop-blur-md flex flex-col items-center justify-center z-10">
                ...
                <p className="mt-4 text-blue-600 font-bold">AI 正在构思中...</p>
              </div>
            )}
          </div>
        </section>
